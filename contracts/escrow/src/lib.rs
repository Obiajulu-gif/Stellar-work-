...
impl EscrowContract {
    ...
    pub fn execute_operation(e: Env, op_id: u64) {
        ...
        e.events().publish(
            (Symbol::new(&e, "operation_executed"),),
            (op_id, now),
        );
    }
    ...
    pub fn cancel_operation(e: Env, caller: Address, op_id: u64) {
        ...
        e.events().publish(
            (Symbol::new(&e, "operation_cancelled"),),
            (op_id, caller),
        );
    }
...
fn apply_operation(e: &Env, operation: &AdminOperation) {
    match operation {
        AdminOperation::UpdateFeeBps(new_fee_bps) => {
            ...
            e.events()
                .publish((Symbol::new(e, "fee_updated"),), (fee,));
        }
...
mod test {
    ...
    fn propose_transfer_admin_via_timelock() {
        ...
        let new_admin = Address::generate(&env);
        let op_id = client
            .propose_operation(&admin, &AdminOperation::TransferAdmin(new_admin.clone()));
    ...
    fn propose_and_execute_add_allowed_token() {
        ...
        let op_id = client
            .propose_operation(&admin, &AdminOperation::AddAllowedToken(new_token.clone()));
    ...
    fn cancel_operation_prevents_execution() {
        ...
        assert!(result.is_err(), "cancelled operation must not be executable");
    ...
    fn update_timelock_delay_via_proposal() {
        ...
        let op_id = client
            .propose_operation(&admin, &AdminOperation::UpdateTimelockDelay(new_delay));
    ...
    fn update_timelock_delay_below_minimum_panics() {
        let (env, client, admin, _, _, _) = setup();
        let op_id = client
            .propose_operation(&admin, &AdminOperation::UpdateTimelockDelay(100u64));
